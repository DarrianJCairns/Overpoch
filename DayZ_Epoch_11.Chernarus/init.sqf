...
dayZ_serverName = "EoD"; //Shown to all players in the bottom left of the screen (country code + server number)
...
DZE_restrictRemoval = ["Plastic_Pole_EP1_DZ","plot_pole_kit","Fence_corrugated_DZ","metal_floor_kit","M240Nest_DZ","ParkBench_DZ","FireBarrel_DZ","Scaffolding_DZ","CanvasHut_DZ","LightPole_DZ","DeerStand_DZ","MetalGate_DZ","StickFence_DZ"]; //Items that can be removed with a crowbar only with proper ownership or access. It is not necessary to add doors, storage or items that inherit from 'ModularItems' to this list. Items that inherit from 'BuiltItems' can be added to this list if desired.Items that inherit from 'BuiltItems' can be added to this list if desired.
...
DZE_PlotPole = [50,70]; // Radius owned by plot pole [Regular objects,Other plotpoles]. Difference between them is the minimum buffer between bases.
...
if (!isDedicated) then {
	...
	_id = player addEventHandler ["Respawn", {_id = [] spawn player_death; _nul = [] execVM "spawn\playerspawn.sqf";}];
	progress_monitor = [] execVM "\z\addons\dayz_code\system\progress_monitor.sqf";
	_nul = [] execVM "spawn\playerspawn.sqf";
	0 cutText ['','BLACK',0];
	0 fadeSound 0;
	0 fadeMusic 0;
};
